...
class SupabaseAdapter implements PersistenceAdapter {
  ...
  async persistRunTrace(run: Record<string, unknown>) {
    const row = {
      ...
      avg_score: (run.avg_score as number) ?? null,
      planning_score: (run.planning_score as number) ?? null,
      strategy_score: (run.strategy_score as number) ?? null,
      ...
      completed_at: (run.completed_at as string) ?? new Date().toISOString(),
